...
test_root="$(mktemp -d "$test_parent/windshare-release-environment.XXXXXXXX")"
...
cleanup() {
  ...
  trap - EXIT
  case "$test_root" in
    "$test_parent"/windshare-release-environment.*) rm -rf -- "$test_root" ;;
...
fail() {
  echo "release Go environment contract: $1" >&2
  exit 1
}
...
assert_failed_preflight_preserves_environment() {
  ...
  if windshare_prepare_release_go_environment "$blocked_root" >/dev/null 2>&1; then
...
windshare_prepare_release_go_environment "$fresh_root"
...
trap - EXIT
rm -rf -- "$test_root"
echo "release Go environment contract: PASS"
