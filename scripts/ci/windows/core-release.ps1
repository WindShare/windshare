...
if ([string]::IsNullOrWhiteSpace($Version) -or $CommitSHA -cnotmatch '^[0-9a-f]{40}$') {
    throw 'usage: scripts/ci/windows/core-release.ps1 -Version <version> -CommitSHA <40-char-sha> [-NativeProfile windows-ntfs]'
}
...
try {
    ...
    Invoke-Step 'exact release checkout contract' {
        & $currentPowerShell -NoLogo -NoProfile -NonInteractive -File `
            (Join-Path $ciRoot 'core-release-checkout.tests.ps1')
    }
    Invoke-Step 'private Go environment contract' {
        & $currentPowerShell -NoLogo -NoProfile -NonInteractive -File `
            (Join-Path $ciRoot 'core-release-environment.tests.ps1')
    }
