...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) (
    'windshare-release-environment-contract-{0}' -f [Guid]::NewGuid().ToString('N')
)
...
try {
    ...
    $state = Enter-ReleaseGoEnvironment -ReleaseRoot $freshRoot
    ...
    Exit-ReleaseGoEnvironment -State $state
    $state = $null
    foreach ($name in $seededNames) {
        $actual = [Environment]::GetEnvironmentVariable(
            $name,
    ...
    Assert-Throws 'pre-existing cache' {
        Enter-ReleaseGoEnvironment -ReleaseRoot $blockedRoot
    }
    ...
    if (Test-Path -LiteralPath $testRoot) {
        ...
        $ownedPrefix = Join-Path $resolvedTemp 'windshare-release-environment-contract-'
...
Write-Output 'release Go environment contract: PASS'
