...
try {
    ...
    [IO.File]::WriteAllText(
        (Join-Path $fixtureRepository 'core\transfer\untracked-after-clean.go'),
        "package transfer`n",
        [Text.UTF8Encoding]::new($false)
    )
    Invoke-Git -Arguments @('-C', $fixtureRepository, 'add', '--', 'core/README.md') | Out-Null

    $environmentState = Enter-CoreReleaseGoEnvironment -ReleaseRoot $releaseEnvironment
    ...
    $actualREADMEObject = [string](Invoke-Git -Arguments @(
        'hash-object', '--', (Join-Path $testRoot 'extracted-core\README.md')
    ))
    if ($actualREADMEObject.Trim() -cne $expectedREADMEObject) {
        throw 'archive consumed tracked worktree/index bytes'
    }
    if (Test-Path -LiteralPath (
        Join-Path $testRoot 'extracted-core\transfer\untracked-after-clean.go'
    )) {
        throw 'archive consumed an untracked worktree file'
    }
} finally {
    if ($null -ne $environmentState) {
        Exit-CoreReleaseGoEnvironment -State $environmentState
    }
    if (Test-Path -LiteralPath $testRoot) {
        $resolvedRoot = [IO.Path]::GetFullPath($testRoot)
        $resolvedTemp = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
        $ownedPrefix = Join-Path $resolvedTemp 'windshare-core-archive-contract-'
...
Write-Output 'core release committed archive contract: PASS'
