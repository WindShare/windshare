...
expect_checkout_failure() {
  ...
  if windshare_assert_exact_release_checkout "$fixture_repository" "$commit_sha" \
...
expect_checkout_failure "tracked mutation"
...
expect_checkout_failure "untracked mutation"
...
expect_checkout_failure "assume-unchanged mutation"
...
expect_checkout_failure "skip-worktree mutation"
grep -Fq 'non-default Git index state' "$test_root/failure.log" ||
