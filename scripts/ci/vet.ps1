# CI-parity vet/build gate (Windows). Mirrors:
#  - the vet + build steps of ci.yml go-root and go-core; the native run below
#    is GOOS=windows, so it simultaneously covers ci.yml windows-tests' vet
#    steps (Windows-tagged files are analyzed, not just compiled).
...
Invoke-Step 'go build (root)' { go build ./... }
Invoke-Step 'go vet (core, GOOS=windows)' { go -C core vet ./... }
Invoke-Step 'go build (core)' { go -C core build ./... }
