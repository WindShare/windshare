...
test_root="$(mktemp -d "$test_parent/windshare-release-checkout.XXXXXXXX")"
...
cleanup() {
  ...
  trap - EXIT
  case "$test_root" in
    "$test_parent"/windshare-release-checkout.*) rm -rf -- "$test_root" ;;
...
fail() {
  echo "release checkout contract: $1" >&2
  exit 1
}
...
trap - EXIT
rm -rf -- "$test_root"
echo "release checkout contract: PASS"
