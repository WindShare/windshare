...
test_root="$(mktemp -d "$test_parent/windshare-core-archive.XXXXXXXX")"
...
cleanup() {
  ...
  trap - EXIT
  case "$test_root" in
    "$test_parent"/windshare-core-archive.*) rm -rf -- "$test_root" ;;
...
fail() {
  echo "core release committed archive contract: $1" >&2
  exit 1
}
...
printf 'package transfer\n' >"$fixture_repository/core/transfer/untracked-after-clean.go"
...
windshare_prepare_core_release_go_environment "$release_environment"
...
[ ! -e "$test_root/extracted-core/transfer/untracked-after-clean.go" ] ||
  fail "archive consumed an untracked worktree file"

trap - EXIT
rm -rf -- "$test_root"
echo "core release committed archive contract: PASS"
