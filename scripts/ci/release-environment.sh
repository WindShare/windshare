...
windshare_prepare_release_go_environment() {
  ...
  if [ -z "$release_root" ] || [ ! -d "$release_root" ] || [ -L "$release_root" ]; then
    echo "release environment requires an existing no-follow release root" >&2
    return 1
  fi
  ...
  for cache_path in "$module_cache_path" "$build_cache_path" "$go_path"; do
    if [ -e "$cache_path" ] || [ -L "$cache_path" ]; then
      echo "release cache path must be fresh: $cache_path" >&2
      return 1
    fi
  done
  for cache_path in "$module_cache_path" "$build_cache_path" "$go_path"; do
