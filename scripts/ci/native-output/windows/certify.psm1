...
$script:CoordinatorReleaseRootLeafPattern = '^windshare-release-[0-9a-f]{32}$'
...
function Initialize-WindowsNativeUserProfileInterop {
    if ($null -eq ('WindShare.Release.NativeUserProfile' -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace WindShare.Release
{
...
function Invoke-WindowsNativeProfileDelete([string]$UserSID, [string]$ProfilePath) {
    ...
    return [WindShare.Release.NativeUserProfile]::Delete($UserSID, $nativeProfilePath)
}
...
function Initialize-WindowsNativeDirectoryInterop {
    if ($null -eq ('WindShare.Release.NativeDirectory' -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
...
function New-WindowsNativeCoordinatorReleaseRootAt(
    ...
    $nativeError = [WindShare.Release.NativeDirectory]::CreateExclusive(
        $rootPath,
        $securityDescriptor
    )
