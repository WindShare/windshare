...
$script:CoreReleaseFixedEnvironment = [ordered]@{
    GOENV       = 'off'
    GOFLAGS     = ''
    GOTOOLCHAIN = 'local'
    GOWORK      = 'off'
...
$script:CoreReleaseClearedEnvironment = @(
    'GOOS',
    'GOARCH',
    'CGO_ENABLED',
    'GOEXPERIMENT'
...
function Set-CoreReleaseProcessEnvironmentValue {
    param(
        [Parameter(Mandatory)]
        [string]$Name,
...
function Enter-CoreReleaseGoEnvironment {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$ReleaseRoot
    ...
    if (-not [IO.Path]::IsPathFullyQualified($ReleaseRoot) -or
        ...
        throw 'core release environment requires an existing absolute release root'
    }
    ...
    if (($rootInfo.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
        throw 'core release environment root must not be a reparse point'
    }
    ...
    foreach ($cachePath in $cacheValues.Values) {
        if (Test-Path -LiteralPath $cachePath) {
            throw "core release cache path must be fresh: $cachePath"
        }
    }

    $variableNames = @($cacheValues.Keys) +
        @($script:CoreReleaseFixedEnvironment.Keys) +
        @($script:CoreReleaseClearedEnvironment)
    ...
    foreach ($entry in $script:CoreReleaseFixedEnvironment.GetEnumerator()) {
        [Environment]::SetEnvironmentVariable(
            $entry.Key,
            $entry.Value,
            [EnvironmentVariableTarget]::Process
    ...
    foreach ($name in $script:CoreReleaseClearedEnvironment) {
        Set-CoreReleaseProcessEnvironmentValue -Name $name -Value $null
    }
...
function Exit-CoreReleaseGoEnvironment {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$State
    ...
    foreach ($name in $State.VariableNames) {
        Set-CoreReleaseProcessEnvironmentValue `
            -Name $name `
...
Export-ModuleMember -Function @(
    'Enter-CoreReleaseGoEnvironment',
    'Exit-CoreReleaseGoEnvironment'
)
