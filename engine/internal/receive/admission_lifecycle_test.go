...
func TestReceiverAdmissionMonitorConsumesFailureBeforeJoinReturns(t *testing.T) {
	...
	monitorDone := (&runner{}).monitorReceiverAdmission(admission, nil, observation, &receiverLocalStop{})
	if err := admission.ObservePeer(receiverPeerFailed); err != nil {
		t.Fatal(err)
	}
	admission.Wait()
	<-monitorDone
	admission.Close()
	if len(warningEvents(*events)) != 1 {
		t.Fatalf("warnings=%v", warningEvents(*events))
	}
	if warning, ok := (warningEvents(*events))[0].(Warning); !ok || !errors.Is(warning.Cause, relay.resumeError) {
		t.Fatalf("warning=%v", (warningEvents(*events))[0])
	}
}
...
func TestReceiverAdmissionMonitorSuppressesFailureAfterRuntimeTerminal(t *testing.T) {
	...
	if len(warningEvents(*events)) != 0 {
		t.Fatalf("late admission warning=%v", warningEvents(*events))
	}
}
...
func TestP2POnlyContentAdmissionMakesPeerLossTerminal(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			monitorDone := (&runner{}).monitorReceiverAdmission(
				admission,
				receiverRuntime,
				observation,
				&receiverLocalStop{},
			...
			if len(warningEvents(*events)) != 1 {
				t.Fatalf("peer unavailable warnings=%v", warningEvents(*events))
			}
			if warning, ok := (warningEvents(*events))[0].(Warning); !ok || warning.Code != FailurePeerStopped {
				t.Fatalf("peer unavailable=%v", (warningEvents(*events))[0])
			}
		})
	}
}
