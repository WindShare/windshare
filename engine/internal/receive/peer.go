...
func (a *runner) monitorReceiverAdmission(
	...
	go func() {
		...
		case receiverAdmissionTerminalP2PUnavailable:
			observation.warningCode(FailurePeerStopped)
		default:
			return
		}
		if runtime != nil {
...
func (a *runner) observeRelayContentAdmission(
...
func (paths *receiverContentPaths) observeContent(activity []transfer.LaneContentActivity, now time.Time) {
	...
	if paths.fallbackPending && paths.relay {
		...
		paths.observation.fallback(FailurePeerStopped)
	}
	if path, changed := paths.changedPathLocked(); changed {
		paths.observation.contentPath(path)
	}
}
func (paths *receiverContentPaths) changedPathLocked() (ContentPath, bool) {
	var current ContentPath
	switch {
	case paths.direct && paths.relay:
		current = ContentPathDirectAndRelay
	case paths.direct:
		current = ContentPathDirect
	case paths.relay:
		current = ContentPathRelay
	default:
		return 0, false
	}
...
type receiverLocalStop struct {
	mu     sync.Mutex
	reason ReceiverLocalStopReason
}

func (stop *receiverLocalStop) record(reason ReceiverLocalStopReason) {
	if stop == nil || reason == ReceiverLocalStopNone {
		return
	}
	stop.mu.Lock()
	if stop.reason == 0 {
...
func (stop *receiverLocalStop) snapshot() ReceiverLocalStopReason {
	if stop == nil {
		return ReceiverLocalStopNone
	}
	stop.mu.Lock()
...
func (peer *activeReceiverPeer) Close() {
	peer.CloseWithReason(ReceiverLocalStopCaller)
}
...
func (peer *activeReceiverPeer) CloseWithReason(reason ReceiverLocalStopReason) {
	if peer == nil {
		return
	}
	peer.once.Do(func() {
...
func (a *runner) startReceiverPeer(
	ctx context.Context,
...
func receiverPeerSetupFailureCode(phase receiverPeerSetupPhase) FailureCode {
	switch phase {
	case receiverPeerSetupFactory:
		return FailurePeerConfiguration
	case receiverPeerSetupSignaling:
		return FailurePeerSignaling
	default:
		return FailurePeerNegotiation
	}
}

func (a *runner) monitorReceiverPeer(
	attempt receiverPeerAttempt,
	runtime receiverRuntimeCloser,
		...
		case <-attempt.Done():
			...
			case receiverPeerSessionUnsafe:
				...
				if err != nil {
					...
					observation.warningCode(FailurePeerProtocol)
				}
				runtime.Close()
				return false
...
func (paths *receiverContentPaths) setTransfer(operation receivecontract.OperationID, job transfer.TransferJobID) {
	paths.mu.Lock()
	paths.operation, paths.job = operation, job
	paths.mu.Unlock()
}
func (paths *receiverContentPaths) transferIdentity() (receivecontract.OperationID, transfer.TransferJobID) {
	paths.mu.Lock()
	defer paths.mu.Unlock()
	return paths.operation, paths.job
}
