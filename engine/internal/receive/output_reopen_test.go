...
func (authority *layoutRecordingGetOutputAuthority) LookupActive(
	...
	*authority.events = append(*authority.events, "lookup")
	return getOutputLookup{Kind: getOutputLookupMiss, Reservation: authority}, nil
}

func (authority *layoutRecordingGetOutputAuthority) Create(
	_ context.Context,
	artifact receivecontract.ArtifactSpec,
	...
	*authority.events = append(*authority.events, "create")
	...
	return getOutputOperation{Intent: intent, Mode: getOutputResumable, Materializer: testUnusedMaterializer{}}, nil
}
...
func TestResolveGetOutputOperationReopensBeforeShapeAndRefusesConcurrentLease(t *testing.T) {
	...
	openAuthority := func() getOutputAuthority {
		authority, err := (FilesystemOutput{RootPath: rootPath, CreateRoot: true}).NewOutputAuthority(OutputConfig{})
		if err != nil {
			t.Fatal(err)
		}
...
type testUnusedMaterializer struct{}

func (testUnusedMaterializer) OpenDirectTree(context.Context, transfer.ReceiveIntent) (transfer.DirectTreeSession, error) {
	return nil, errors.New("unexpected materializer open")
}
