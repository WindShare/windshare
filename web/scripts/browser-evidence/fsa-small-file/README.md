...
Product success requires exact `Published` DirectTree output and median authority-to-publication at most 15 seconds. The finite positive product/baseline median ratio is diagnostic only. Further benchmark-specific tuning is intentionally stopped because protecting common-case behavior and maintainability takes priority. `-DiagnosticProductOnly` captures one warm-up plus one measured product run without making a paired target claim. Raw results stay below `evidence/<session-id>/`. Browser profiles and materialized trees stay below repository `.tmp/fsa-small-file-native/<session-id>/` so Go package discovery skips them.
