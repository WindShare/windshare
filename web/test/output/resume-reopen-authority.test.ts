...
function retainedCleanupBackend(
...
function requiredDescriptor(lifecycle: ReceiveLifecycleState, now: number) {
...
function resumableReceive(intent: ReceiveIntent, generation: bigint): Extract<
...
async function seedWorkspaceAdmission(
  state: MemoryRepositoryState,
...
async function seedWorkspaceZipAdmission(
  state: MemoryRepositoryState,
  intent: ReceiveIntent,
...
async function seedWorkspacePackage(state: MemoryRepositoryState, intent: ReceiveIntent) {
...
async function workspaceIntent(): Promise<ReceiveIntent> {
...
async function workspaceZipIntent(): Promise<ReceiveIntent> {
...
function workspaceZipPreparationInput(intent: ReceiveIntent) {
...
function memoryStorage(root: MemoryDirectoryHandle): NonNullable<
...
function bytesFilled(seed: number): (length: number) => Uint8Array {
