...
test('task progress counts written bytes while discovery is open and uses an exact denominator afterwards', async ({ page }) => {
  ...
  await update({ receivedObjectBytes: 0n, writtenBytes: 0n, materializedBytes: 0n })
  await page.clock.runFor(1000)
  for (let second = 1; second <= 4; second++) {
    await update({ receivedObjectBytes: BigInt(second) * MIB, writtenBytes: BigInt(second) * MIB, materializedBytes: BigInt(second) * MIB })
    await page.clock.runFor(1000)
  }
  await expect(task.getByText(/(?:KiB|MiB)\/s/)).toBeVisible()
