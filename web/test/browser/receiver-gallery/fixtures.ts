...
export const SCENARIOS = ['folder', 'full-directory', 'portrait', 'landscape', 'video', 'unsupported', 'reconnecting', 'capacity', 'verifying', 'partial-ready', 'browser-handoff', 'saved-cleanup'] as const
...
export async function gallerySnapshot(scenario: Scenario): Promise<V2ReceiverSnapshot> {
  ...
  const directoryRows = scenario === 'full-directory' ? FULL_DIRECTORY_ROWS : ROOT_ROWS
  ...
  return {
    ...
    retained: { kind: 'ready', pending: null, error: null, operations: [0, 1].map(index => ({
      ...
      continuation: 'save-artifact', actions: ['save'], shareInstance: identity(1),
    })) },
  }
}
