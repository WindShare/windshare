...
test('delivered downloads keep their result and offer one-click repetition on desktop and mobile', async ({ page }) => {
  ...
  for (const width of [1280, 390]) {
    ...
    for (const kind of ['published', 'download-started'] as const) {
      ...
      const details = page.locator('.task-details')
      await expect(details).toContainText('Elapsed')
      await expect(details).toContainText('2 min 5 sec')
      await expect(details).toContainText('including pauses and reconnecting')
