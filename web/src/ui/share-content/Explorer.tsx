...
function SelectionCheckbox({ row, toggle, active }: { readonly row: V2BrowseRow; readonly toggle: () => void; readonly active: boolean }) {
  ...
  return <input ref={input} type="checkbox" aria-label={`Select ${row.name}`}
    checked={checked} aria-checked={mixed ? 'mixed' : checked}
    onChange={toggle} />
}

function entryMetadata(row: V2BrowseRow): string {
...
export function Explorer({ rows, breadcrumbs, pageIndex, pageCount, omittedCount, browse, draft, actions }: {
  ...
  return <section className="explorer" aria-label="Shared files" aria-busy={browse.kind === 'loading'}>
    <div className="explorer-toolbar">
      ...
      <button type="button" onClick={draft.mode === 'selection' ? actions.exitSelection : actions.enterSelection}>
    ...
    {rows.length > 0 && <>
      <div className="explorer-columns" aria-hidden="true"><span>Name</span><span>Size</span></div>
      <ul className="explorer-list" tabIndex={0} aria-label="Folder contents">
        {rows.map(row => <li key={row.id} className="explorer-row">
          ...
          <span className={`entry-icon entry-icon-${row.kind}`} aria-hidden="true">{row.kind === 'directory' ? '▰' : '▤'}</span>
          <button type="button" className="entry-name" title={row.name}
            onClick={() => row.kind === 'directory' ? actions.openDirectory(row.id) : actions.preview(row.id)}>
            {row.name}
          </button>
          <span className="entry-metadata">{entryMetadata(row)}</span>
    ...
    {pageCount > 1 && <nav className="directory-pagination" aria-label="Directory pages">
      <button type="button" disabled={pageIndex === 0} onClick={() => actions.showPage(pageIndex - 1)}>Previous</button>
      <span>Page {pageIndex + 1} of {pageCount}</span>
      <button type="button" disabled={pageIndex + 1 >= pageCount} onClick={() => actions.showPage(pageIndex + 1)}>Next</button>
    </nav>}
  </section>
}
