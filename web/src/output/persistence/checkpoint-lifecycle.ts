...
export const FILE_CHECKPOINT_MAX_QUARANTINE_REASON =
...
export const FILE_CHECKPOINT_MAX_RETIREMENT_REASON =
  FILE_CHECKPOINT_RETIREMENT_INVALIDATED_REVISION

export type FileCheckpointPhase =
  | typeof FILE_CHECKPOINT_PHASE_RESERVED
  | typeof FILE_CHECKPOINT_PHASE_ACTIVE
  | typeof FILE_CHECKPOINT_PHASE_PAUSED
  | typeof FILE_CHECKPOINT_PHASE_PUBLISHING
  | typeof FILE_CHECKPOINT_PHASE_PUBLISHED
  | typeof FILE_CHECKPOINT_PHASE_QUARANTINED
  | typeof FILE_CHECKPOINT_PHASE_RETIRED

export type FileCheckpointCommitState =
  | typeof FILE_CHECKPOINT_COMMIT_CANDIDATE
  | typeof FILE_CHECKPOINT_COMMIT_VERIFIED
  | typeof FILE_CHECKPOINT_COMMIT_PUBLISHED
  | typeof FILE_CHECKPOINT_COMMIT_QUARANTINED

export type FileCheckpointQuarantineReason =
  | typeof FILE_CHECKPOINT_QUARANTINE_ANCHOR_MISSING
  | typeof FILE_CHECKPOINT_QUARANTINE_ANCHOR_UNSAFE
  | typeof FILE_CHECKPOINT_QUARANTINE_STAGE_MISSING
  | typeof FILE_CHECKPOINT_QUARANTINE_STAGE_MISMATCH
  | typeof FILE_CHECKPOINT_QUARANTINE_STAGE_UNSAFE
  | typeof FILE_CHECKPOINT_QUARANTINE_FINAL_MISMATCH
  | typeof FILE_CHECKPOINT_QUARANTINE_FINAL_UNSAFE
  | typeof FILE_CHECKPOINT_QUARANTINE_PARTIAL_OBJECT_CREATION
  | typeof FILE_CHECKPOINT_QUARANTINE_PUBLICATION_HISTORY
  | typeof FILE_CHECKPOINT_QUARANTINE_METADATA_MISMATCH
  | typeof FILE_CHECKPOINT_QUARANTINE_UPDATE_TEMPORARY
  | typeof FILE_CHECKPOINT_QUARANTINE_OUTPUT_OBJECT_DUPLICATE

export type FileCheckpointQuarantineOrigin =
  | typeof FILE_CHECKPOINT_ORIGIN_RESERVED
  | typeof FILE_CHECKPOINT_ORIGIN_WITNESSED
  | typeof FILE_CHECKPOINT_ORIGIN_PUBLISHING
  | typeof FILE_CHECKPOINT_ORIGIN_PUBLISH_BLOCKED
  | typeof FILE_CHECKPOINT_ORIGIN_PUBLISHED
  | typeof FILE_CHECKPOINT_ORIGIN_RETIRING

export type FileCheckpointRetirementReason =
  | typeof FILE_CHECKPOINT_RETIREMENT_PUBLISHED
  | typeof FILE_CHECKPOINT_RETIREMENT_ISOLATED_FAILURE
  | typeof FILE_CHECKPOINT_RETIREMENT_PRE_OBJECT_COLLISION
...
export function checkpointPhase(value: number): FileCheckpointPhase {
  if (Number.isInteger(value) &&
      value >= FILE_CHECKPOINT_PHASE_RESERVED && value <= FILE_CHECKPOINT_PHASE_RETIRED) {
    return value as FileCheckpointPhase
  }
  throw new FileCheckpointError('invalid', 'checkpoint phase is invalid')
}

export function checkpointCommitState(value: number): FileCheckpointCommitState {
  if (Number.isInteger(value) &&
      value >= FILE_CHECKPOINT_COMMIT_CANDIDATE &&
      value <= FILE_CHECKPOINT_COMMIT_QUARANTINED) {
    return value as FileCheckpointCommitState
  }
  throw new FileCheckpointError('invalid', 'checkpoint commit state is invalid')
}

export function checkpointLifecycleClaim<T extends number>(
  value: number,
  maximum: number,
  label: string,
): T | 0 {
...
export function validateCheckpointLifecycle(
  ...
  validatePhaseCommitPair(phase, commitState)
  validateLifecycleHistory(phase, quarantineReason, quarantineOrigin, retirementReason)
}
...
export function validCheckpointLifecycleTransition(
  ...
  if (!phaseCommitPairIsValid(next.phase, next.commitState)) return false
  switch (previous.phase) {
    case FILE_CHECKPOINT_PHASE_RESERVED:
      return next.phase === FILE_CHECKPOINT_PHASE_ACTIVE ||
        next.phase === FILE_CHECKPOINT_PHASE_RETIRED ||
        next.phase === FILE_CHECKPOINT_PHASE_QUARANTINED
    case FILE_CHECKPOINT_PHASE_ACTIVE:
      return next.phase === FILE_CHECKPOINT_PHASE_PAUSED ||
        next.phase === FILE_CHECKPOINT_PHASE_PUBLISHING ||
        next.phase === FILE_CHECKPOINT_PHASE_RETIRED ||
        next.phase === FILE_CHECKPOINT_PHASE_QUARANTINED
    ...
    case FILE_CHECKPOINT_PHASE_PUBLISHING:
      return next.phase === FILE_CHECKPOINT_PHASE_PAUSED ||
        next.phase === FILE_CHECKPOINT_PHASE_PUBLISHED ||
        next.phase === FILE_CHECKPOINT_PHASE_RETIRED ||
        next.phase === FILE_CHECKPOINT_PHASE_QUARANTINED
    default:
      return false
...
function validatePhaseCommitPair(
  phase: FileCheckpointPhase,
  commitState: FileCheckpointCommitState,
): void {
  if (!phaseCommitPairIsValid(phase, commitState)) {
    throw new FileCheckpointError('invalid', 'checkpoint phase and commit state disagree')
  }
}

function phaseCommitPairIsValid(
  phase: FileCheckpointPhase,
  commitState: FileCheckpointCommitState,
): boolean {
  switch (commitState) {
    case FILE_CHECKPOINT_COMMIT_CANDIDATE:
      return phase >= FILE_CHECKPOINT_PHASE_RESERVED &&
        phase <= FILE_CHECKPOINT_PHASE_PUBLISHING
    case FILE_CHECKPOINT_COMMIT_VERIFIED:
      return phase >= FILE_CHECKPOINT_PHASE_RESERVED &&
        phase <= FILE_CHECKPOINT_PHASE_PUBLISHING || phase === FILE_CHECKPOINT_PHASE_RETIRED
    case FILE_CHECKPOINT_COMMIT_PUBLISHED:
      return phase === FILE_CHECKPOINT_PHASE_PUBLISHED
    case FILE_CHECKPOINT_COMMIT_QUARANTINED:
      return phase === FILE_CHECKPOINT_PHASE_QUARANTINED
  }
}
...
function validateLifecycleHistory(
  ...
  if (phase === FILE_CHECKPOINT_PHASE_QUARANTINED) {
    const retirementValid = retirementReason >= 1 &&
      retirementReason <= FILE_CHECKPOINT_MAX_RETIREMENT_REASON
    if (quarantineReason < 1 || quarantineReason > FILE_CHECKPOINT_MAX_QUARANTINE_REASON ||
      ...
      throw new FileCheckpointError('invalid', 'checkpoint quarantined history is invalid')
    }
    return
  }
  if (quarantineReason !== 0 || quarantineOrigin !== 0) {
    throw new FileCheckpointError('invalid', 'checkpoint quarantine history requires quarantine')
  }
  if (phase === FILE_CHECKPOINT_PHASE_RETIRED) {
    if (retirementReason < 1 || retirementReason > FILE_CHECKPOINT_MAX_RETIREMENT_REASON) {
      throw new FileCheckpointError('invalid', 'checkpoint retirement history is invalid')
    }
  } else if (retirementReason !== 0) {
    throw new FileCheckpointError('invalid', 'checkpoint retirement history requires retirement')
  }
}
...
function validQuarantineHistory(origin: number, reason: number): boolean {
  switch (reason) {
    case FILE_CHECKPOINT_QUARANTINE_ANCHOR_MISSING:
      return origin >= FILE_CHECKPOINT_ORIGIN_WITNESSED &&
        origin <= FILE_CHECKPOINT_ORIGIN_PUBLISHED
    case FILE_CHECKPOINT_QUARANTINE_STAGE_MISSING:
      return origin >= FILE_CHECKPOINT_ORIGIN_WITNESSED &&
        origin <= FILE_CHECKPOINT_ORIGIN_PUBLISH_BLOCKED
    ...
    case FILE_CHECKPOINT_QUARANTINE_PARTIAL_OBJECT_CREATION:
      return origin === FILE_CHECKPOINT_ORIGIN_RESERVED ||
        origin === FILE_CHECKPOINT_ORIGIN_RETIRING
    case FILE_CHECKPOINT_QUARANTINE_PUBLICATION_HISTORY:
      return origin >= FILE_CHECKPOINT_ORIGIN_RESERVED &&
        origin <= FILE_CHECKPOINT_ORIGIN_PUBLISH_BLOCKED
    case FILE_CHECKPOINT_QUARANTINE_METADATA_MISMATCH:
      return origin === FILE_CHECKPOINT_ORIGIN_PUBLISHING ||
        origin === FILE_CHECKPOINT_ORIGIN_PUBLISHED
    default:
      return reason >= FILE_CHECKPOINT_QUARANTINE_ANCHOR_UNSAFE &&
        reason <= FILE_CHECKPOINT_MAX_QUARANTINE_REASON &&
        origin >= FILE_CHECKPOINT_ORIGIN_RESERVED && origin <= FILE_CHECKPOINT_ORIGIN_RETIRING
  }
}
