...
export type LifecycleEvent =
  ...
  | Readonly<LifecycleEventAuthority & { kind: 'publication-unknown'; lastVerifiedRecordDigest: string }>
  | Readonly<LifecycleEventAuthority & {
  ...
  | Readonly<LifecycleEventAuthority & { kind: 'handoff-unknown'; lastVerifiedRecordDigest: string }>
  | Readonly<LifecycleEventAuthority & {
  ...
  | Readonly<LifecycleEventAuthority & { kind: 'cleanup-unknown'; lastVerifiedRecordDigest: string }>
  | Readonly<LifecycleEventAuthority & {
      kind: 'expiry-observed'
      expiryReceiptDigest: string
      cleanupState: 'clean' | 'cleanup-pending'
    }>
  | Readonly<LifecycleEventAuthority & { kind: 'ownership-unknown'; lastVerifiedRecordDigest: string }>
...
export function reduceReceiveLifecycle(
  ...
  if (event.expectedGeneration !== state.generation || event.leaseId !== context.activeLeaseId ||
      activeLeaseMismatch(state, context.activeLeaseId)) {
