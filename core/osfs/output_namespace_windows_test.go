...
func TestWindowsFilesystemPathFactsUseNativeSemantics(t *testing.T) {
	...
	if !isPathTooLongError(errors.Join(errors.New("context"), syscall.ENAMETOOLONG)) ||
		!isPathTooLongError(windowsErrorFilenameExceedsRange) || isPathTooLongError(syscall.EINVAL) {
