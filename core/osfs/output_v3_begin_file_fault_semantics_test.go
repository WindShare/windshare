...
func TestOutputV3BeginFileFailsBeforeContentWhenAuthorityCannotBeProven(t *testing.T) {
	...
	for _, test := range []struct {
		name      string
		configure func(*testing.T, *filesystemOutputSession, transfer.OutputFile)
		cause     error
		...
		{
			name: "closed session",
			configure: func(t *testing.T, session *filesystemOutputSession, _ transfer.OutputFile) {
				session.mu.Lock()
				session.closed = true
				session.mu.Unlock()
				t.Cleanup(func() {
			...
			cause: errOutputSessionClosed, scope: transfer.OutputFaultSession, code: transfer.OutputFaultOwnership,
		},
		{
			...
			configure: func(t *testing.T, session *filesystemOutputSession, _ transfer.OutputFile) {
				...
				session.filesDir = &outputV3BeginFileStateDirectory{
					outputV3Directory: original, classifyErr: failure,
				}
		...
		},
		{
			...
			configure: func(t *testing.T, session *filesystemOutputSession, _ transfer.OutputFile) {
				original := session.filesDir
				session.filesDir = &outputV3BeginFileStateDirectory{
					outputV3Directory: original, createDirectoryErr: failure,
				}
				t.Cleanup(func() { session.filesDir = original })
			},
			cause: failure, code: transfer.OutputFaultStateIO,
		},
		{
			...
			configure: func(_ *testing.T, session *filesystemOutputSession, _ transfer.OutputFile) {
		...
		},
		{
			...
			configure: func(t *testing.T, session *filesystemOutputSession, file transfer.OutputFile) {
				...
				original := session.filesDir
				session.filesDir = &outputV3BeginFileStateDirectory{
					outputV3Directory: original, childCreateFileErr: failure,
				}
				t.Cleanup(func() { session.filesDir = original })
			},
			cause: failure, code: transfer.OutputFaultStateIO,
...
func TestOutputV3DirectoryLifecyclePropagatesSessionAndNamespaceFaults(t *testing.T) {
	...
	for _, operation := range []struct {
		...
		t.Run(operation.name+" closed session", func(t *testing.T) {
			...
			opened.Session.mu.Unlock()
			if !errors.Is(err, errOutputSessionClosed) {
...
func TestOutputV3ObjectAllocationBudgetRejectsNonAuthorityIDs(t *testing.T) {
	...
	opened.Session.owner.objectIDs = &outputV3SemanticObjectIDs{
		values: make([]resumestate.OutputObjectID, outputStateAllocationAttempts),
	}
...
func outputV3InstallBeginFileRootFault(
	...
	session.platform = &outputV3PublicationPlatform{
		outputV3Platform: original,
		root: &outputV3PublicationDirectory{
			outputV3Directory: original.Root(),
			faults:            faults,
		},
	}
