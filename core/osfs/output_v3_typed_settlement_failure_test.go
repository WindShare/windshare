...
func TestOutputV3ExpiredSettlementContextsRemainTyped(t *testing.T) {
	...
	pauseTransaction := v3RecoveryBeginTransaction(t, opened.Session, file).(*filesystemFileTransaction)
...
func TestOutputV3RecoveryFailureClassifierKeepsEvidenceAndDenialDistinct(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "positive entry marker", cause: errors.Join(errOutputV3PositiveEntryEvidence, denied), boundary: outputV3BeforeEntryEvidence, want: outputV3RecoveryAmbiguous},
		{name: "identity contradiction", cause: errors.Join(errOutputV3Unsafe, denied), boundary: outputV3AuthorizedMutation, want: outputV3RecoveryAmbiguous},
		{name: "fixed-name collision", cause: errOutputV3Collision, boundary: outputV3AuthorizedMutation, want: outputV3RecoveryAmbiguous},
	} {
	...
	ambiguousErr := recoveryFileOutputFault("recover", errOutputV3Unsafe, outputV3AuthorizedMutation)
...
func outputV3CommitError(transaction *filesystemFileTransaction, ctx context.Context) error {
