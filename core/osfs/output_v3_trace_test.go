...
func TestOutputV3FileSettlementTraceProjectsEveryTypedSettlement(t *testing.T) {
	...
	session := &filesystemOutputSession{
		owner:        &FilesystemOutputAuthority{tracer: recorder},
...
func TestOutputV3SettlementTraceRunsAfterJoinedPauseCleanup(t *testing.T) {
	...
	transaction.data = &outputV3SemanticFaultFile{
		outputV3File: transaction.data, syncErr: syncFailure, closeErr: closeFailure,
	}
...
func (*outputV3TraceTestLock) File() outputV3File { return nil }
...
func TestOutputV3NativeLockTraceReportsEachAcquireOutcomeOnce(t *testing.T) {
	for _, scopeTest := range []struct {
		...
		{"coordinator", FilesystemOutputNativeLockCoordinator, transfer.OutputFaultRoot, rootOutputFault("unexpected coordinator lock", errOutputRootUnsafe)},
		{"session", FilesystemOutputNativeLockSession, transfer.OutputFaultSession, intentOutputFault("unexpected session lock", errOutputIntentUnsafe)},
	} {
		for _, outcomeTest := range []struct {
			...
			{
				name: "acquired", acquire: func() (outputV3Lock, bool, error) {
					return &outputV3TraceTestLock{}, false, nil
				}, wantMilestone: FilesystemOutputNativeLockAcquired,
			},
			{
				name: "contended", acquire: func() (outputV3Lock, bool, error) {
					return nil, false, errors.Join(errOutputV3LockBusy, errors.New("busy"))
				}, wantMilestone: FilesystemOutputNativeLockContended,
			},
			{
				name: "acquire failed", acquire: func() (outputV3Lock, bool, error) {
			...
			t.Run(scopeTest.name+"/"+outcomeTest.name, func(t *testing.T) {
				recorder := &outputV3TraceRecorder{}
				authority := &FilesystemOutputAuthority{tracer: recorder}
...
func TestOutputV3NativeLockTraceClosesUnexpectedCreatedLock(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			authority := &FilesystemOutputAuthority{tracer: recorder}
			lock, err := authority.acquireRuntimeNativeLock(
				func() (outputV3Lock, bool, error) { return raw, true, nil },
				filesystemOutputNativeLockContext{
					scope: FilesystemOutputNativeLockCoordinator, failureScope: transfer.OutputFaultRoot,
				},
				rootOutputFault("unexpected created lock", errOutputRootUnsafe),
			)
...
func TestOutputV3NativeLockTraceReportsOneTerminalEventUnderConcurrentClose(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			authority := &FilesystemOutputAuthority{tracer: recorder}
			lock, err := authority.acquireRuntimeNativeLock(
				func() (outputV3Lock, bool, error) { return raw, false, nil },
				filesystemOutputNativeLockContext{
					scope: FilesystemOutputNativeLockSession, failureScope: transfer.OutputFaultSession,
				},
				intentOutputFault("unexpected session lock", errOutputIntentUnsafe),
			)
			if err != nil {
				t.Fatal(err)
			}
...
func TestOutputV3ContendingOpenerTracesNoFalseSessionOwnership(t *testing.T) {
	...
	if _, err := v3OpenSelection(context.Background(), contender, selection); !errors.Is(err, errOutputSessionActive) {
