...
func TestOutputV3BoundaryListEmptyRootIsReadOnlyAndRejectsUninstalledCandidate(t *testing.T) {
	...
	failed, err := authority.listResumeState(context.Background(), FilesystemResumeRoot{RootPath: root})
	if failed != nil {
...
func TestOutputV3BoundaryCanceledDiscardDoesNotConsumeReference(t *testing.T) {
	...
	after, err := authority.listResumeState(context.Background(), FilesystemResumeRoot{RootPath: root})
	if err != nil {
		t.Fatal(err)
	}
...
func TestOutputV3BoundaryMultipleSessionsRemainIndividuallyDiscardable(t *testing.T) {
	...
	// Public open refuses a second session for one intent. Creating the second
	// valid namespace directly models a persisted external/concurrent cut so the
	// read-only inventory and explicit-discard boundaries can be exercised.
	_, second, created, err := authority.createSessionDirectory(
		intent, control.control, selection,
		v3RecoveryAncestryBinding(t, control.control.OutputRoot(), selection),
	)
	if err != nil || !created {
		t.Fatalf("create second fixed session = created %t, error %v", created, err)
	}
	if err := errors.Join(second.Close(), intent.Close(), control.Close(), platform.Close()); err != nil {
		t.Fatal(err)
	}
	...
	for _, summary := range summaries {
		if summary.Reference.Kind() != ResumeStateNeedsAttention ||
			!v3RecoveryHasAttention(summary, "multiple-sessions-for-intent") {
...
func TestOutputV3BoundaryStaticAdmissionPrecedesPlatformAuthority(t *testing.T) {
	...
	authority.platformFactory = func(string, bool) (outputV3Platform, error) {
		platformCalls++
		return nil, errors.Join(errOutputV3Unsupported, errors.New("injected unsupported platform"))
	}
...
func TestOutputV3BoundaryForcedDirectoryPinRevokesAllRetains(t *testing.T) {
	...
	directory := &outputV3BoundaryCountingDirectory{outputV3Directory: duplicate}
