		...
		t.Run(test.name, func(t *testing.T) {
			...
			opened.Session.platform = &outputV3PublicationPlatform{
				...
				root: &outputV3PublicationDirectory{
					outputV3Directory: originalPlatform.Root(), faults: &faults,
				},
			}
...
func TestOutputV3FreshAdmissionRejectsGuardRootDisplacementBeforeMaterialization(t *testing.T) {
	...
	authority.platformFactory = func(path string, create bool) (outputV3Platform, error) {
		...
		displaced, err := openOutputV3Platform(displacedRoot, false)
		if err != nil {
...
type outputV3AdmissionDisplacementPlatform struct {
	outputV3Platform
	displaced     outputV3Platform
	guardAcquired bool
}

func (platform *outputV3AdmissionDisplacementPlatform) Root() outputV3Directory {
...
func (platform *outputV3AdmissionDisplacementPlatform) Close() error {
	return errors.Join(platform.outputV3Platform.Close(), platform.displaced.Close())
}
