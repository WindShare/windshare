	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			opened.Session.sessionDir = &stateStoreFaultDirectory{
				outputV3Directory: opened.Session.sessionDir,
				fault:             test.fault,
			...
			for index, transaction := range transactions {
				if err := transaction.WriteRange(context.Background(), 0, []byte{1}); !errors.Is(err, errOutputSessionClosed) {
...
func TestOutputV3ConcurrentWriteWaitingAtPoisonCannotWrite(t *testing.T) {
	...
	writes := &atomic.Int64{}
	writer.data = &v3CountingOutputFile{outputV3File: writer.data, writes: writes}
	writer.mu.Lock()
	...
	poisoned.recordDir = &stateStoreFaultDirectory{
		outputV3Directory: poisoned.recordDir,
...
func waitForV3OperationReader(t *testing.T, session *filesystemOutputSession) {
	t.Helper()
...
func requireV3OwnerReleased(t *testing.T, session *filesystemOutputSession) {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		session.mu.Lock()
