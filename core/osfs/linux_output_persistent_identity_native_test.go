...
func TestLinuxExt4RestartIdentityRejectsForcedInodeReuse(t *testing.T) {
	...
	for index := uint64(0); index < freeAfterTarget; index++ {
	...
	previous := initial
	for cycle := 0; cycle < linuxNativeReuseCycles; cycle++ {
...
func TestLinuxExt4RestartIdentityWorker(t *testing.T) {
	...
	platformValue, err := openOutputV3Platform(path, false)
	if err != nil {
		t.Fatal(err)
	}
...
func assertLinuxNativeWorkerBoundary(t *testing.T) {
	...
	for _, line := range strings.Split(string(status), "\n") {
		if strings.HasPrefix(line, "CapEff:") {
			capabilityHex = strings.TrimSpace(strings.TrimPrefix(line, "CapEff:"))
			break
		}
	}
