...
func TestLinuxSelectionMetadataPolicyIsNotPublishedBeforeProbeSuccess(t *testing.T) {
	...
	if err := platform.ValidateSelectionMetadata(selection); !errors.Is(err, errOutputV3Unsupported) {
...
func TestLinuxSelectionMetadataPolicyProvesExactSelectedDirectoryInode(t *testing.T) {
	...
	if err := platform.ValidateSelectionMetadata(selection); !errors.Is(err, errOutputV3Unsafe) {
	...
	if err := directory.ValidateMetadataAuthority(); !errors.Is(err, errOutputV3Unsupported) {
...
func linuxSelectionMetadataPlan(
	...
	for index, directory := range directories {
		selectedDirectories[index] = transfer.OutputSelectionDirectory{
			Path:         directory.path,
			DirectoryID:  v3RecoveryIdentity16[catalog.DirectoryID](byte(0xb0 + index)),
			Generation:   v3RecoveryIdentity16[catalog.DirectoryGeneration](byte(0xc0 + index)),
	...
	for index, file := range files {
		selectedFiles[index] = transfer.OutputSelectionFile{
			...
			FileID:            v3RecoveryIdentity16[catalog.FileID](byte(0xa0 + index)),
