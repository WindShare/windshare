...
func TestOutputV3RecoversInterruptedHeaderReplacementBeforeExactChildValidation(t *testing.T) {
	...
	temporaryName, err := opened.Session.store.temporaryName(resumestate.HeaderRecordName)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOutputV3CorruptHeaderIsIntentScopedAndExplicitlyDiscardable(t *testing.T) {
	...
	if !v3RecoveryHasAttention(summaries[0], "session-header-corrupt") {
...
func TestOutputV3CorruptControlBlocksTheRootAndCannotBeDiscardedThroughSessionRef(t *testing.T) {
	...
	beforeInventory, err := authority.listResumeState(context.Background(), FilesystemResumeRoot{RootPath: root})
	if err != nil {
		t.Fatal(err)
	}
	...
	failedInventory, err := authority.listResumeState(context.Background(), FilesystemResumeRoot{RootPath: root})
...
func v3RecoveryHasAttention(summary ResumeStateSummary, code string) bool {
