...
func TestOutputV3FileStateGatePreservesCanonicalRecordAuthority(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "canonical record wrong type", faults: outputV3FileShardFaults{observeOverrideAt: 1, observeKind: outputcap.EntryDirectory}, wantQuarantine: true},
		{name: "missing target with update", faults: outputV3FileShardFaults{observeOverrideAt: 1, observeKind: outputcap.EntryAbsent}, appendTemporary: true, wantQuarantine: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			faulted := &outputV3FileShardDirectory{Directory: duplicate, faults: &faults}
			owned := true
			defer func() {
				if owned {
					_ = faulted.Close()
...
func (directory *outputV3FileShardParent) OpenDirectory(
	...
	return &outputV3FileShardDirectory{Directory: opened, faults: directory.faults}, nil
}
...
func (directory *outputV3FileShardDirectory) Close() error {
	return errors.Join(directory.Directory.Close(), directory.faults.directoryCloseErr)
}
...
func (directory *outputV3FileShardDirectory) ObserveEntry(name string) (outputcap.EntryKind, error) {
...
func (directory *outputV3FileShardDirectory) OpenFile(
	...
	return &outputV3FileShardFile{File: opened, faults: directory.faults}, nil
}

func (directory *outputV3FileShardDirectory) RemoveFile(name string, expected outputcap.File) error {
...
func outputV3CreateStateTemporary(t *testing.T, shard outputcap.Directory, name string) {
	t.Helper()
...
func outputV3FailureRequiresJobPause(err error) bool {
	...
	case interface{ Unwrap() []error }:
		return slices.ContainsFunc(wrapped.Unwrap(), outputV3FailureRequiresJobPause)
	case interface{ Unwrap() error }:
