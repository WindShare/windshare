...
func TestOutputV3RejectsRangeOverlapBeforeMutatingCheckpointAuthority(t *testing.T) {
	...
	concrete := resumed.(*FileTransaction)
...
func TestOutputV3PublishingTransactionHasExactlyOneTerminalClaimant(t *testing.T) {
	...
	if phase != resumestate.FilePublishing || lifecycle != FileTransactionSettling {
		t.Fatalf("blocked commit state = (phase=%v, lifecycle=%v)", phase, lifecycle)
	}
	if _, err := transaction.Checkpoint(context.Background()); !errors.Is(err, outputfault.ErrSessionClosed) {
		t.Fatalf("checkpoint during publishing error = %v, want transaction ownership rejection", err)
	}
	if _, err := transaction.Pause(context.Background(), transfer.FilePauseInterrupted); !errors.Is(err, outputfault.ErrSessionClosed) {
		t.Fatalf("competing pause error = %v, want transaction ownership rejection", err)
	}
	if _, err := transaction.Commit(context.Background()); !errors.Is(err, outputfault.ErrSessionClosed) {
	...
	if lifecycle != FileTransactionClosed || active != 0 {
...
func requireOutputV3TransactionBytes(
	...
	read, err := transaction.data.file.ReadAt(actual, 0)
