...
func TestOutputV3RevisionOnlyMismatchRetiresOldAuthorityWithoutRangeReuse(t *testing.T) {
	...
	oldTransaction := v3RecoveryBeginTransaction(t, opened.Session, oldFile).(*FileTransaction)
	...
	replacement := transaction.(*FileTransaction)
	...
	for _, finalState := range []string{"missing", "matching", "foreign"} {
		t.Run(finalState, func(t *testing.T) {
			...
			case "missing":
				...
				outputV3AbandonTransaction(t, transaction.(*FileTransaction))
				outputV3AssertObjectNamesAbsent(t, root, selection, reopened.Session.SessionID(), oldRecord)
			case "matching":
...
func TestOutputV3PublishingRevisionMismatchClassifiesFinalParentReopenFailure(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "unsafe identity quarantines", cause: errors.Join(outputcap.ErrUnsafeNamespace, errors.New("identity mismatch")), quarantine: true},
	} {
...
func (platform *outputV3InvalidatedFinalReplacementPlatform) Root() outputcap.Directory {
	return outputV3WrapInvalidatedFinalReplacementDirectory(platform.Platform.Root(), platform.gate)
}
...
func outputV3WrapInvalidatedFinalReplacementDirectory(
	...
	return &outputV3InvalidatedFinalReplacementDirectory{Directory: directory, gate: gate}
}

func outputV3UnwrapInvalidatedFinalReplacementDirectory(directory outputcap.Directory) outputcap.Directory {
...
func (directory *outputV3InvalidatedFinalReplacementDirectory) Duplicate() (outputcap.Directory, error) {
...
func (directory *outputV3InvalidatedFinalReplacementDirectory) SameDirectory(
	other outputcap.Directory,
) (bool, error) {
	return directory.Directory.SameDirectory(
...
func (directory *outputV3InvalidatedFinalReplacementDirectory) ValidateCreateAuthority() error {
	return validateOutputCreateAuthority(directory.Directory)
}

func (directory *outputV3InvalidatedFinalReplacementDirectory) ValidateMetadataAuthority() error {
	return validateOutputMetadataAuthority(directory.Directory)
}
...
func outputV3AbandonTransaction(t *testing.T, transaction *FileTransaction) {
	t.Helper()
