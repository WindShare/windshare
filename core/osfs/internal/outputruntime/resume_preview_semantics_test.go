...
func TestOutputV3ResumeAPIsRejectUnboundAndCanceledCapabilities(t *testing.T) {
	...
	if inventory, err := authority.ListResumeState(context.Background(), ""); inventory != nil ||
		!errors.Is(err, transfer.ErrInvalidOutputBinding) {
		t.Fatalf("unbound root listing = (%v, %v)", inventory, err)
	}
	canceled, cancel := context.WithCancel(context.Background())
	cancel()
	if inventory, err := authority.ListResumeState(canceled, root); inventory != nil ||
		!errors.Is(err, context.Canceled) {
		t.Fatalf("canceled listing = (%v, %v)", inventory, err)
	}

	if settlement, err := (*Authority)(nil).DiscardResumeState(
...
func TestOutputV3PrivateTreePreviewEnforcesBudgetsBeforeTraversal(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !errors.Is(err, outputfault.ErrInspectionLimit) {
...
func TestOutputV3PrivateTreePreviewPreservesUnsafeEntryEvidence(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: previewEntry(outputcap.EntryRegularFile, 3),
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: previewEntry(outputcap.EntryRegularFile, 4),
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: previewEntry(outputcap.EntryRegularFile, 1),
			},
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: &outputV3ResumePreviewEntry{
					kind: outputcap.EntryRegularFile, allocatedErr: injected,
				}, matches: true,
			},
			cause: injected,
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: &outputV3ResumePreviewEntry{
					kind: outputcap.EntryRegularFile, closeErr: injected,
				}, matches: true,
			},
			cause: injected,
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"entry"}, entry: previewEntry(outputcap.EntryRegularFile, 1), matches: true,
			},
			allocated: math.MaxUint64, cause: outputcap.ErrUnsafeNamespace,
		},
		{
			name: "file record accounting",
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"record.state"}, entry: previewEntry(outputcap.EntryRegularFile, 5), matches: true,
			},
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"directory"}, entry: previewEntry(outputcap.EntryDirectory, 0),
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"opaque"}, entry: previewEntry(outputcap.EntryOther, 0),
			},
			wantAttention: 1,
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"opaque"}, entry: &outputV3ResumePreviewEntry{
					kind: outputcap.EntryOther, closeErr: injected,
				},
			},
			cause: injected,
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				names: []string{"missing"}, entry: previewEntry(outputcap.EntryAbsent, 0),
			},
			cause: outputcap.ErrUnsafeNamespace,
		},
	} {
...
func TestOutputV3PrivateEntryRemovalRequiresLiveAuthority(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0)},
		},
		{
			name:      "unknown pinned entry kind",
			directory: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryKind(0xff), 0)},
			cause:     outputcap.ErrUnsafeNamespace,
		},
		{
			name:      "regular entry authority changed",
			directory: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryRegularFile, 0)},
		...
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				entry: previewEntry(outputcap.EntryDirectory, 0), publicChildErr: injected,
			},
			cause: injected,
		},
		{
			...
			directory: &outputV3ResumePreviewDirectory{
				entry: previewEntry(outputcap.EntryDirectory, 0), publicChild: &outputV3ResumePreviewDirectory{},
				matches: true,
			},
			depth: resumestate.MaxStateNestingDepth, cause: outputfault.ErrInspectionLimit,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
...
type outputV3ResumePreviewDirectory struct {
	...
	matches         bool
	matchErr        error
	privateChild    outputcap.Directory
	privateChildErr error
	publicChild     outputcap.Directory
...
func (directory *outputV3ResumePreviewDirectory) OpenEntry(string) (outputcap.CurrentEntryReference, error) {
...
func (directory *outputV3ResumePreviewDirectory) RemoveEntry(_ string, _ outputcap.CurrentEntryReference) error {
	directory.removeCalls++
	return directory.removeErr
}

func (directory *outputV3ResumePreviewDirectory) RemoveFile(_ string, _ outputcap.File) error {
...
func (entry *outputV3ResumePreviewEntry) Kind() outputcap.EntryKind { return entry.kind }
...
func previewEntry(kind outputcap.EntryKind, allocated uint64) outputcap.CurrentEntryReference {
