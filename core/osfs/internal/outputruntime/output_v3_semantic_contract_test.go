...
func TestOutputV3NestedDirectoryLifecycleAndPreObjectCollision(t *testing.T) {
	...
	if err := (*Session)(nil).FinalizeDirectory(context.Background(), directory); !errors.Is(err, transfer.ErrInvalidOutputBinding) {
...
func TestOutputV3AuthorityPrimitivesRejectTamperingAndReleasePins(t *testing.T) {
	...
	entry := &v3SemanticEntryRef{kind: outputcap.EntryDirectory}
	...
	closing := &v3SemanticEntryRef{kind: outputcap.EntryRegularFile, closeErr: closeCause}
	...
	countingRoot := &v3SemanticCountingDirectory{Directory: duplicate}
...
func (entry *v3SemanticEntryRef) Kind() outputcap.EntryKind { return entry.kind }
...
func TestOutputV3OpaqueInventoryRemovesOnlyPinnedSessionEntry(t *testing.T) {
	...
	if malformed == nil || malformed.Reference.Kind() != ResumeStateOpaqueUnsafe ||
		!runtimeSemanticHasAttention(*malformed, "malformed-session-namespace") {
		t.Fatalf("malformed pinned session summary = %+v", malformed)
	}
	if empty == nil || empty.Reference.Kind() != ResumeStateOpaqueUnsafe ||
		!runtimeSemanticHasAttention(*empty, "empty-resume-namespace") {
...
func runtimeSemanticHasAttention(summary ResumeStateSummary, expected string) bool {
...
var _ outputcap.CurrentEntryReference = (*v3SemanticEntryRef)(nil)
