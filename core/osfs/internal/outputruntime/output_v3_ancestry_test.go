...
func TestOutputAncestryOperationFaultSeparatesDenialFromContradiction(t *testing.T) {
	...
	if !errors.As(denied, &deniedFault) || deniedFault.Scope() != transfer.OutputFaultSession ||
		...
		errors.Is(denied, outputfault.ErrIntentUnsafe) {
	...
	if !errors.As(unsafe, &unsafeFault) || unsafeFault.Scope() != transfer.OutputFaultSession ||
		...
		!errors.Is(unsafe, outputfault.ErrIntentUnsafe) {
...
func (directory *outputAncestryTestDirectory) SameDirectory(other outputcap.Directory) (bool, error) {
	current, ok := other.(*outputAncestryTestDirectory)
	return ok && current.node == directory.node, nil
}

func (directory *outputAncestryTestDirectory) OpenDirectory(name string, _ bool) (outputcap.Directory, error) {
...
func (directory *outputAncestryTestDirectory) PrepareIdentityClaim() (outputcap.PersistentDirectoryIdentity, error) {
	directory.node.prepareCalls++
	directory.node.prepared = true
	return outputcap.NewPersistentDirectoryIdentity(directory.node.claim), nil
}

func (directory *outputAncestryTestDirectory) IdentityClaim() (outputcap.PersistentDirectoryIdentity, error) {
	directory.node.identityCalls++
	if !directory.node.prepared {
		return outputcap.PersistentDirectoryIdentity{}, errors.New("identity claim is not prepared")
	}
	return outputcap.NewPersistentDirectoryIdentity(directory.node.claim), nil
}
...
var _ outputcap.Platform = (*outputAncestryPlatformDecorator)(nil)
...
func (platform *outputAncestryTestPlatform) Root() outputcap.Directory { return platform.root }
...
func (platform *outputAncestryTestPlatform) AcquirePublicOperationGuard() (outputcap.PublicOperationGuard, error) {
...
func (guard *outputAncestryTestGuard) Root() outputcap.Directory { return guard.platform.root }
