...
func stateStoreHeaderImages(t *testing.T) (
	...
	encode := func(header resumestate.Header) (outputnamespace.RecordImage, []byte) {
		encoded, encodeErr := resumestate.EncodeHeader(header)
		if encodeErr != nil {
			t.Fatal(encodeErr)
		}
		return outputnamespace.NewRecordImage(encoded, header.StateGeneration()), encoded
	}
	currentImage, currentEncoded := encode(namespace.Header())
	nextImage, _ := encode(next.Header())
	divergentImage, _ := encode(divergent.Header())
	return currentImage, nextImage, divergentImage, currentEncoded
}
...
func stateStoreReplacementFixture(
	...
	store := outputnamespace.NewStore(outputnamespace.StoreConfig{
		Random: bytes.NewReader(bytes.Repeat([]byte{0xa2}, 256)),
	})
...
const (
	...
	stateStoreFaultLink
	stateStoreFaultLinkAfterMutation
	stateStoreFaultLinkAfterMutationAndParentSync
...
func (directory *stateStoreFaultDirectory) LinkFileNoReplace(source outputcap.File, name string) (outputcap.File, error) {
	...
	target := &stateStoreFaultFile{File: linked, owner: directory, name: name, linkedTarget: true}
	if directory.fault == stateStoreFaultLinkAfterMutation ||
		directory.fault == stateStoreFaultLinkAfterMutationAndParentSync {
		// Return the installed target with the reported failure so Store must settle
		// the mutation by reopening the exact fixed-name image.
		return target, errStateStoreInjected
	}
	return target, nil
}
...
func (directory *stateStoreFaultDirectory) Sync() error {
	...
	if directory.linkAttempted {
		directory.linkSyncs++
		if (directory.fault == stateStoreFaultCreateParentSync ||
			directory.fault == stateStoreFaultLinkAfterMutationAndParentSync) && directory.linkSyncs == 1 {
			return errStateStoreInjected
		}
