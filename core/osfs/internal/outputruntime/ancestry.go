...
var (
	...
	errOutputAncestryAuthorityDenied = outputfault.ErrAncestryAuthorityDenied
)
...
type outputAncestryEntry struct {
	path  string
	claim outputcap.PersistentDirectoryIdentity
}
...
func outputAncestryTraceDecision(err error) FilesystemOutputAncestryDecision {
	...
	case errors.Is(err, errOutputAncestryUnsafe), errors.Is(err, outputcap.ErrUnsafeNamespace):
...
func (authority *Authority) traceOutputAncestry(
...
func (session *Session) traceOutputAncestry(
	boundary FilesystemOutputAncestryBoundary,
	locator resumestate.LocatorDigest,
	err error,
) {
...
func (snapshot outputAncestrySnapshot) claim(path string) (outputcap.PersistentDirectoryIdentity, bool) {
	...
	if !found {
		return outputcap.PersistentDirectoryIdentity{}, false
	}
	return snapshot.entries[index].claim, true
}
...
func (snapshot outputAncestrySnapshot) matches(expected outputAncestrySnapshot) bool {
	...
	for index := range snapshot.entries {
		if snapshot.entries[index].path != expected.entries[index].path ||
			!snapshot.entries[index].claim.Equal(expected.entries[index].claim) {
			return false
		}
	}
	return true
...
type outputAncestryValidation struct {
	...
	directories map[string]outputcap.Directory
	openedPaths []string
}

func (validation *outputAncestryValidation) directory(path string) (outputcap.Directory, error) {
...
func captureOutputSelectionAncestryWithGuardedPreparation(
	...
	selection transfer.OutputSelection,
	requirement outputAncestryRequirement,
	prepare func(outputcap.Directory) error,
) (*outputAncestryValidation, error) {
...
func collectOutputAncestry(
	...
	selection transfer.OutputSelection,
	requirement outputAncestryRequirement,
) (outputAncestrySnapshot, map[string]outputcap.Directory, []string, error) {
	paths, err := canonicalOutputAncestryPaths(selection)
	if err != nil {
		return outputAncestrySnapshot{}, nil, nil, err
	}
	directories := make(map[string]outputcap.Directory, len(paths))
	...
	for _, path := range paths {
		...
		claims = append(claims, resumestate.OutputAncestryIdentityClaim{
			CanonicalPath: path, IdentityClaim: identity.Bytes(),
		})
...
func classifyOutputAncestryEvidence(err error) error {
	...
	if errors.Is(err, outputcap.ErrUnsafeNamespace) && !errors.Is(err, errOutputAncestryAuthorityDenied) {
