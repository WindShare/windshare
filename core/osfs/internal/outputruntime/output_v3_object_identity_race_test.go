...
func TestOutputV3ConcurrentFilesCannotPersistTheSameOutputObject(t *testing.T) {
	...
	for index, file := range selection.Files() {
		...
		concrete := transaction.(*FileTransaction)
...
func v3RecoveryCreateObjectShard(t *testing.T, parent outputcap.Directory, name string) {
	t.Helper()
...
func v3RecoveryAbandonStartedFile(
	...
	concrete.lifecycle = FileTransactionClosed
...
func (platform *v3RecoveryObjectPlatform) Root() outputcap.Directory { return platform.root }
...
func v3RecoveryWrapObjectDirectory(
	...
	return &v3RecoveryObjectDirectory{Directory: directory, gate: gate}
}

func v3RecoveryUnwrapObjectDirectory(directory outputcap.Directory) outputcap.Directory {
...
func (directory *v3RecoveryObjectDirectory) Duplicate() (outputcap.Directory, error) {
	duplicate, err := directory.Directory.Duplicate()
	return v3RecoveryWrapObjectDirectory(duplicate, directory.gate), err
}

func (directory *v3RecoveryObjectDirectory) SameDirectory(other outputcap.Directory) (bool, error) {
	return directory.Directory.SameDirectory(v3RecoveryUnwrapObjectDirectory(other))
}

func (directory *v3RecoveryObjectDirectory) ObserveEntry(name string) (outputcap.EntryKind, error) {
	kind, err := directory.Directory.ObserveEntry(name)
	if err == nil && kind == outputcap.EntryAbsent && name == directory.gate.target {
