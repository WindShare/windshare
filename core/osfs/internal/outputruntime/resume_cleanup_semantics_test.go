...
func TestOutputV3OpaqueDiscardRequiresLiveEntryAuthority(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "missing pin", cause: outputfault.ErrIntentUnsafe},
		{name: "absent pin", entry: previewEntry(outputcap.EntryAbsent, 0), cause: outputfault.ErrIntentUnsafe},
		{name: "directory pin", entry: previewEntry(outputcap.EntryDirectory, 0), cause: outputfault.ErrIntentUnsafe},
		{name: "allocated size failure", entry: &outputV3ResumePreviewEntry{kind: outputcap.EntryRegularFile, allocatedErr: failure}, cause: failure},
		{name: "authority changed", entry: previewEntry(outputcap.EntryRegularFile, 1), verifyErr: failure, cause: failure},
		{name: "fixed removal failure", entry: previewEntry(outputcap.EntryRegularFile, 2), removeErr: failure, cause: failure},
		{name: "opaque regular file", entry: previewEntry(outputcap.EntryRegularFile, 7), wantBytes: 7, wantRemoved: true},
		{name: "opaque special entry", entry: previewEntry(outputcap.EntryOther, 9), wantBytes: 9, wantRemoved: true},
	} {
...
func TestOutputV3EmptyIntentRemovalRevalidatesPinnedDirectory(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "identity mismatch", parent: outputV3ResumeCleanupDirectoryFaults{forceDifferent: true}, cause: outputfault.ErrRootUnsafe},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			faults := test.parent
			parent := &outputV3ResumeCleanupDirectory{Directory: session.control.Sessions(), faults: &faults}
			...
			if (kind == outputcap.EntryAbsent) != test.wantAbsent {
				t.Fatalf("intent absence = %t, want %t", kind == outputcap.EntryAbsent, test.wantAbsent)
			}
		})
	}
}
...
func TestOutputV3EmptySessionShellRequiresVerifiedEmptyIdentity(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "nonempty shell", session: &outputV3ResumePreviewDirectory{names: []string{"remaining"}}, intent: &outputV3ResumePreviewDirectory{}, cause: outputfault.ErrIntentUnsafe},
...
func TestOutputV3CorruptDiscardStopsAtEveryAuthorityBoundary(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "session enumeration", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0), namesErr: failure}, cause: failure},
		{name: "extra child parent verification", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0), names: []string{"extra"}}, verifyAt: 2, cause: failure},
		{name: "lock parent verification", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0)}, lock: &outputV3ResumeCleanupLock{file: &outputV3ResumePreviewFile{}}, verifyAt: 2, cause: failure},
		{name: "lock without file", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0)}, lock: &outputV3ResumeCleanupLock{}, cause: outputfault.ErrIntentUnsafe},
		{name: "lock removal failure", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0), removeFileErr: failure}, lock: &outputV3ResumeCleanupLock{file: &outputV3ResumePreviewFile{}}, cause: failure},
		{name: "lock sync failure", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0), syncErr: failure}, lock: &outputV3ResumeCleanupLock{file: &outputV3ResumePreviewFile{}}, cause: failure},
		{name: "lock close failure", session: &outputV3ResumePreviewDirectory{entry: previewEntry(outputcap.EntryAbsent, 0)}, lock: &outputV3ResumeCleanupLock{file: &outputV3ResumePreviewFile{}, closeErr: failure}, cause: failure},
	} {
		t.Run(test.name, func(t *testing.T) {
			verifyCalls := 0
			err := recoverCorruptDiscard(
				&outputnamespace.ControlNamespace{}, nil, test.session,
				ResumeStateRef{}, previewEntry(outputcap.EntryDirectory, 0), test.lock,
				func() error {
					verifyCalls++
...
func (directory *outputV3ResumeCleanupDirectory) OpenDirectory(
	...
	return &outputV3ResumeCleanupDirectory{Directory: opened, faults: directory.faults}, nil
}

func (directory *outputV3ResumeCleanupDirectory) SameDirectory(other outputcap.Directory) (bool, error) {
...
func (directory *outputV3ResumeCleanupDirectory) Close() error {
	return errors.Join(directory.Directory.Close(), directory.faults.childCloseErr)
}
...
func (lock *outputV3ResumeCleanupLock) File() outputcap.File { return lock.file }
func (lock *outputV3ResumeCleanupLock) Close() error         { return lock.closeErr }

func outputV3ResumeCleanupIntent(
	t *testing.T,
) (*Session, string, outputcap.Directory) {
	t.Helper()
