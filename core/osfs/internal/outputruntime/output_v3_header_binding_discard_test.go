...
func TestOutputV3ExplicitDiscardRemovesDecodableHeaderWithForeignBinding(t *testing.T) {
	...
	if summaries[0].Reference.Kind() != ResumeStateNeedsAttention ||
		!runtimeHeaderBindingHasAttention(summaries[0], "session-header-binding") {
...
func runtimeHeaderBindingHasAttention(summary ResumeStateSummary, expected string) bool {
