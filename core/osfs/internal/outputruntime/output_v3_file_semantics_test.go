...
func TestOutputV3BeginFileRejectsInvalidAndDuplicateOwnershipClaims(t *testing.T) {
	...
	if !errors.Is(err, outputfault.ErrFileActive) {
...
func TestOutputV3ActiveRetirementPausesWhenAStageLosesItsAnchor(t *testing.T) {
	...
	file := v3RecoveryOutputFile(t, opened.Session, selection, 1)
	transaction := v3RecoveryBeginTransaction(t, opened.Session, file).(*FileTransaction)
	if err := transaction.anchorDir.RemoveFile(transaction.anchorName, transaction.anchor.file); err != nil {
		t.Fatal(err)
	}
...
func TestOutputV3TerminalCloseFailureIsCleanupStateIO(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			transaction.mu.Lock()
			transaction.data = stagedData{file: &outputV3PublicationFile{
				File: transaction.data.file,
				faults: outputV3PublicationFileFaults{
					closeErr: errors.Join(outputcap.ErrUnsafeNamespace, cleanupCause),
				},
			}}
			...
			if !errors.Is(resultErr, cleanupCause) || !errors.Is(resultErr, outputcap.ErrUnsafeNamespace) ||
...
func TestOutputV3TransactionStartRejectsPostObservationWitnessRaces(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			opened := v3RecoveryOpen(t, v3RecoveryAuthority(t, root, sessionIDs), root, selection)
			file := v3RecoveryOutputFile(t, opened.Session, selection, 1)
			initial := v3RecoveryBeginTransaction(t, opened.Session, file).(*FileTransaction)
			...
			case "stage-shard":
				opened.Session.stagesDir = &outputV3SemanticFaultShardParent{
					Directory: originalStages, absentShard: stageName.Shard(),
				}
			case "anchor-shard":
				opened.Session.anchorsDir = &outputV3SemanticFaultShardParent{
					Directory: originalAnchors, absentShard: anchorName.Shard(),
				}
			case "stage-open":
				opened.Session.stagesDir = &outputV3SemanticFaultShardParent{
					Directory: originalStages, targetShard: stageName.Shard(), childOpenFileErr: failure,
				}
			case "anchor-open":
				opened.Session.anchorsDir = &outputV3SemanticFaultShardParent{
					Directory: originalAnchors, targetShard: anchorName.Shard(), childOpenFileErr: failure,
				}
			case "identity":
				opened.Session.stagesDir = &outputV3SemanticFaultShardParent{
					Directory: originalStages, targetShard: stageName.Shard(), childForceDifferent: true,
				}
			}
...
func TestOutputV3CheckpointFailuresPreserveLastVerifiedAuthority(t *testing.T) {
	...
	transaction.data = stagedData{file: &outputV3SemanticFaultFile{File: originalData.file, shortWrite: true}}
	...
	transaction.data = stagedData{file: &outputV3SemanticFaultFile{File: originalData.file, syncErr: syncFailure}}
...
func TestOutputV3CheckpointWitnessComparisonSeparatesDenialFromContradiction(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "explicit unsafe quarantines", cause: errors.Join(outputcap.ErrUnsafeNamespace, errors.New("identity unavailable")), quarantine: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			transaction.data = stagedData{file: &outputV3SemanticFaultFile{
				File: originalData.file, sameErr: test.cause, forceDifferent: test.different,
			}}
...
func TestOutputV3WitnessCreationCleanupCannotChooseRecoveryTaxonomy(t *testing.T) {
	...
	unsafeOperation := errors.Join(outputcap.ErrUnsafeNamespace, unsafeCause)
	unsafeCleanup := errors.Join(outputcap.ErrUnsafeNamespace, cleanupCause)
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			anchorFaults := &outputV3WitnessCreationDirectory{
				Directory:      opened.Session.anchorsDir,
...
func TestOutputV3MetadataFailureResumesCompleteCheckpoint(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			faultData := &outputV3SemanticFaultFile{File: transaction.data.file}
			test.configure(faultData, metadataFailure)
			transaction.data = stagedData{file: faultData}
...
func TestOutputV3TransactionRejectsBoundsAndInvalidSettlementsWithoutMutation(t *testing.T) {
	...
	for _, write := range []struct {
		...
		if err := transaction.WriteRange(context.Background(), write.offset, write.data); !errors.Is(err, outputfault.ErrOutOfRange) {
...
func TestOutputV3PublicationWitnessCleanupClosesEveryHandleOnce(t *testing.T) {
	...
	witness := &publicationWitness{
		stage: stagedData{file: stage}, anchor: anchorWitness{file: anchor},
	}
	...
	if err := (*publicationWitness)(nil).Close(); err != nil {
