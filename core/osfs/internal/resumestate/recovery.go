...
func internalFileObservationQuarantine(
	...
	if observation.Stage == EntryNotObserved {
		...
		if observation.Anchor == AnchorMissing && phase != FileReserved {
	...
	if observation.Anchor == AnchorVerified && observation.Stage == EntryPresentUnresolved ||
		observation.Anchor != AnchorVerified &&
			(observation.Stage == EntrySameAsAnchor || observation.Stage == EntryDifferentFromAnchor) {
		return 0, false
	}
	if observation.Anchor == AnchorUnsafe {
		return QuarantineAnchorUnsafe, true
	...
	if observation.Anchor == AnchorMissing {
		if phase == FileReserved && observation.Stage == EntryMissing {
			return 0, false
		}
		if phase == FileReserved && observation.Stage == EntryPresentUnresolved {
			return QuarantinePartialObjectCreation, true
		}
		return QuarantineAnchorMissing, true
	}
	if observation.Stage == EntryDifferentFromAnchor || observation.Stage == EntryPresentUnresolved {
