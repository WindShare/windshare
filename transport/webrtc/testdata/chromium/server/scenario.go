...
func (s *interopServer) saturateOutbound(channel *windwebrtc.Channel, raw *pion.DataChannel) error {
	if err := channel.Send(context.Background(), patternedFrame(serverProbeMarker, session.MaxFrameSize)); err != nil {
	...
	burst := patternedFrame(serverBurstMarker, session.MaxFrameSize)
...
func (s *interopServer) runHappyScenario(channel *windwebrtc.Channel) {
	...
	if err := channel.Send(context.Background(), session.Frame{serverFinishedMarker}); err != nil {
...
func (s *interopServer) runCancellationScenario(channel *windwebrtc.Channel) {
	...
	defer cancel()
	result, waitObserved := s.startObservedSend(baseContext, channel, session.Frame{canceledSendMarker})
	...
	// Ordered delivery of this barrier before the terminal lets Chromium prove
	// the canceled marker was never enqueued, without an absence timeout.
	if err := channel.Send(context.Background(), session.Frame{cancellationBarrier}); err != nil {
...
func (s *interopServer) runRemoteCloseScenario(channel *windwebrtc.Channel) {
	result, waitObserved := s.startObservedSend(context.Background(), channel, session.Frame{remoteCloseMarker})
	if err := waitForCapacityObservation(waitObserved, result); err != nil {
	...
	s.mu.Unlock()
	if channel.State() != session.Closed || !errors.Is(channelErr, windwebrtc.ErrRemoteClosed) {
...
func (s *interopServer) sendTerminalAndSettle(channel *windwebrtc.Channel) {
	...
	s.result.Events = append(s.result.Events, "channel-done")
	s.mu.Unlock()
	if channel.State() != session.Closed || channelErr != nil {
