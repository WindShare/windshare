...
const (
	...
	terminationExitCodeKeyBytes                 = 32
...
func generateTerminationExitCodes() (terminationExitCodes, error) {
	return generateTerminationExitCodesFrom(rand.Reader)
}

func generateTerminationExitCodesFrom(entropy io.Reader) (terminationExitCodes, error) {
	key := make([]byte, terminationExitCodeKeyBytes)
	if _, err := io.ReadFull(entropy, key); err != nil {
		return terminationExitCodes{}, fmt.Errorf("read private termination-code entropy: %w", err)
	}
	defer clear(key)
...
func reconcileTerminationIntervention(
	...
	return ownerprotocol.TerminationNatural, false, nil
}
...
func terminateAfterAuthorityFailure(job jobLifecycleAuthority, request supervisionRequest, cause error) error {
	return terminateAfterAuthorityFailureWithStart(job, request, cause, targetStartUnknown)

}

func terminateAfterStartedAuthorityFailure(job jobLifecycleAuthority, request supervisionRequest, cause error) error {
	return terminateAfterAuthorityFailureWithStart(job, request, cause, targetKnownStarted)
}

func terminateAfterUnstartedAuthorityFailure(job jobLifecycleAuthority, request supervisionRequest, cause error) error {
	return terminateAfterAuthorityFailureWithStart(job, request, cause, targetKnownUnstarted)
}

func terminateAfterAuthorityFailureWithStart(
	job jobLifecycleAuthority,
	request supervisionRequest,
	cause error,
	start targetStartKnowledge,
) error {
	terminationErr := job.terminate(job.exitCodes().authority)
	cleanupErr := waitForJobEmpty(job, time.Duration(request.TerminationGraceMilliseconds)*time.Millisecond)
	return &authorityTerminationError{
		cause: cause, terminationErr: terminationErr, cleanupErr: cleanupErr, start: start,
	}
}
