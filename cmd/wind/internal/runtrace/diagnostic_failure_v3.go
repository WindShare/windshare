...
type diagnosticFailureV3 struct {
	Source         string                  `json:"source"`
	ErrorAvailable bool                    `json:"error_available"`
	Nodes          []diagnosticErrorNodeV3 `json:"nodes"`
	CaptureStack   []diagnosticFrameV3     `json:"capture_stack"`
	Truncated      bool                    `json:"truncated"`
}

type diagnosticErrorNodeV3 struct {
...
type diagnosticFrameV3 struct {
	Function string `json:"function"`
	File     string `json:"file"`
	Line     string `json:"line"`
}

func projectDiagnosticFailure(snapshot diagnosticerror.Snapshot) *diagnosticFailureV3 {
	if !snapshot.Present() {
		return nil
	}
	nodes := snapshot.Nodes()
	failure := &diagnosticFailureV3{
		Source: snapshot.Source(), ErrorAvailable: len(nodes) != 0,
		Nodes:        make([]diagnosticErrorNodeV3, 0, len(nodes)),
		CaptureStack: make([]diagnosticFrameV3, 0),
	...
	for _, node := range nodes {
		...
		failure.Nodes = append(failure.Nodes, diagnosticErrorNodeV3{
	...
	for _, frame := range snapshot.CaptureStack() {
		failure.CaptureStack = append(failure.CaptureStack, diagnosticFrameV3{
