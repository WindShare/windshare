...
func NewProgressSnapshot(spec ProgressSpec) (ProgressSnapshot, error) {
	...
	if spec.CountersExact {
		...
		publishedOutcomes, publishedOverflow := checkedAdd(publishedOutcomes, spec.FileOutcomes.PreviouslyPublishedFiles)
		completedBytes, completedOverflow := checkedAdd(spec.VerifiedBytes, spec.PreviouslyPublishedBytes)
		if overflow || publishedOverflow || completedOverflow || !validExactItemBlockCounts(spec.FileOutcomes) ||
			spec.NewlyVerifiedBytes > spec.VerifiedBytes ||
			completedBytes > spec.DiscoveredBytes ||
			spec.PreviouslyPublishedBytes > spec.PublishedBytes ||
			spec.PublishedBytes > completedBytes ||
	...
	return ProgressSnapshot{
		...
		previouslyPublishedBytes: spec.PreviouslyPublishedBytes,
...
func (snapshot ProgressSnapshot) PreviouslyPublishedBytes() uint64 {
	return snapshot.previouslyPublishedBytes
}
func (snapshot ProgressSnapshot) CompletedBytes() uint64 {
	completed, overflow := checkedAdd(snapshot.verifiedBytes, snapshot.previouslyPublishedBytes)
	if overflow {
		return ^uint64(0)
	}
	return completed
...
func (snapshot ProgressSnapshot) Valid() bool {
	_, err := NewProgressSnapshot(ProgressSpec{
		...
		PreviouslyPublishedBytes: snapshot.previouslyPublishedBytes,
