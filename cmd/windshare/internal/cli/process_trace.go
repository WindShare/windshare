...
func newProcessTraceWithSink(
	...
	if err != nil {
		return nil, fmt.Errorf("windshare: load test trace context: %w", err)
	}
	if !present {
		return nil, nil
	}
	if openSink == nil {
		return nil, errors.New("windshare: private test event sink opener is nil")
	}
	events, err := openSink(operation.EventIdentity())
	if err != nil {
		return nil, fmt.Errorf("windshare: open private test event sink: %w", err)
	}
	if events == nil {
		return nil, errors.New("windshare: private test event sink opener returned nil")
	}
	...
	for _, component := range []testrun.Component{processTraceShareComponent, processTraceGetComponent} {
		...
		if recorderErr != nil {
			return nil, errors.Join(
				fmt.Errorf("windshare: create %s test event recorder: %w", component, recorderErr),
				events.Close(),
			)
		}
