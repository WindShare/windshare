...
func TestClosedEnumRegistriesNameEveryDeclaredValue(t *testing.T) {
	tests := []struct {
		...
		{"message key", int(MessageUnexpected), int(MessageTraceIncomplete), func(value int) (string, bool) { return SafeMessageKey(value).Name() }},
		...
		{"relay lifecycle stage", int(RelayTerminalReserved), int(RelayLinkClosed), func(value int) (string, bool) { return RelayLifecycleStage(value).Name() }},
