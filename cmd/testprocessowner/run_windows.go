...
func runPlatform(arguments []string, config processowner.Config) error {
	files, err := inheritedFiles(arguments, "handle")
	if err != nil {
		return err
	}
	defer files.close()
	return windowsjob.Run(config, files.status, files.control, files.events)
}
...
func inheritedFiles(arguments []string, suffix string) (ownerFiles, error) {
	...
	for index, name := range wanted {
		...
		value, parseErr := strconv.ParseUint(arguments[index*2+1], 10, strconv.IntSize)
		handle := windows.Handle(uintptr(value))
		if parseErr != nil || handle == 0 || handle == windows.InvalidHandle {
			return ownerFiles{}, fmt.Errorf("process-owner option %s is invalid", name)
		}
		*targets[index] = os.NewFile(uintptr(handle), name)
