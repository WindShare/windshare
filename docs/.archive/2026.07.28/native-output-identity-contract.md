...
Linux/ext4 keeps three identity roles separate. Current handles compare the live mount/device/inode/type tuple while those handles pin the object. A file witness is the live stage/anchor handle pair backed by the private hard-link namespace and bound state record; it is not a reusable inode value. Persisted directory identity combines the certified mount instance (unique mount ID, device, runtime filesystem ID, and the ext4 filesystem UUID returned by `FS_IOC_GETFSUUID`) with directory type/inode and exact [`STATX_BTIME`](https://man7.org/linux/man-pages/man2/statx.2.html) seconds/nanoseconds. Device plus inode is never sufficient. A nonzero `FS_IOC_GETVERSION` result is encoded only as additive evidence; zero or an unsupported get-version operation is accepted as absent evidence, and WindShare never sets a generation.

The Linux release remains blocked unless the tiny-inode cross-process reuse gate passes this exact production identity. This candidate has no xattr fallback and never silently degrades to device-plus-inode or best-effort resume.

Linux external placement ancestors above the output root may be owned only by UID 0 or the effective receiver UID; the output root and every in-root selected-closure directory must be receiver-owned. Certification rejects an ambiguous POSIX ACL, any group/other write-plus-search (`W+X`) authority, and every shared sticky ancestor including `/tmp`. Every persisted directory identity requires handle-bound `STATX_BTIME`; independently, a file or directory needing nanoseconds or seconds outside signed 32-bit range uses that field to prove the required ext4 timestamp layout. Ext4 directories also reject casefold, fscrypt, project-inherit, immutable, and append-only flags. Probe mutations are synced before exact comparison. Final validation plus `linkat` is not claimed atomic against a hostile same-UID process.
...
- Linux/ext4 rejection of missing filesystem UUID, `STATX_BTIME`, or unique mount identity; acceptance of zero/unavailable generation as absent evidence; binding of optional nonzero generation; forced same-inode replacement across processes; and rejection of device-plus-inode-only identity, external ownership other than UID 0/receiver, non-receiver-owned in-root closure, ambiguous ACL, group/other `W+X`, and shared sticky ancestry including `/tmp`;
...
- non-skippable real process-kill recovery on Windows/local NTFS as a standard user, and on a constructed tiny-inode loop-ext4 chroot where separate receiver-UID processes have zero effective capabilities and must detect every forced same-inode replacement;
- successful completion/discard leaving no session object, with only valid fixed global control metadata remaining.

No unmount/remount, reboot, OS-crash, or power-loss test may be described as passing the product contract until that durability level is separately certified.
