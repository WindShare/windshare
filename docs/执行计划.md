...
| **M1c** P2P 与并发 | P2P 直连 + 热切换 + 并发 | `transport/webrtc`、10s 赛跑 + 热切换、并发多接收端验证、性能基线 |
...
> 本文详述 **M1(a/b/c)**;M0 为前置任务。三个子阶段各有独立验收门(见 [§11](#11-m1-任务分解wbs))。
...
windshare/                         # 仓库 github.com/windshare/windshare(组织 WindShare;GitHub 大小写不敏感,模块路径取小写惯例)
  ...
  web/                             # 模块外:TS 网页接收端(windshare.top 前端):Vite + React + TS
    ...
    src/manifest/                  # 清单 CBOR 解析(与 core/manifest 对齐)
...
- **块 i(block)**:
  ...
  ```
  线传输时把 `blockCT` 整体按 `MaxFrameSize` 切帧,nonce 自然位于首帧——**数据面帧布局无需新增 nonce 字段**(见 [§6.7](#67-线协议信令面--数据面))。
...
- **schema 演进次序**:解码**先宽容探测 `v`**——版本不识别 → 明确报错"清单版本不支持,请升级";版本已知才按对应 schema 严格解码(B15)。否则旧接收端遇新清单只会抛不可读的 CBOR 结构错误。
...
func (r *Receiver) ChunksFor(paths []string) []uint64  // 选择性下载:选中 → 所需块号
func (r *Receiver) Accept(i uint64, blockCT []byte) error // 取 nonce → chunk.Open → WriteRange
func (r *Receiver) Missing() []uint64               // 断点:还缺哪些块
```

- **调度器独占数据面协议**:在一组 `FrameChannel`(P2P + relay 可并存)之上维护"需求集 = 选中块 − Have()"、每通道请求队列与在途块、超时/重试/重排、通道评分(吞吐/RTT)、帧→块重组、背压。
- **有序交付模式(流式 Sink)**:FSA 随机写不依赖顺序,但单文件流与流式 ZIP(§6.10)必须按流顺序出字节。`Sink` 可声明有序约束;调度器据此按块号升序消费需求集,维护有界重排缓冲(≤ 在途窗口 × 块大小),且**最小未交付块号的重试永远最高优先**——否则队头块丢失会堵死缓冲(队头阻塞换个地方复活)。
- **收尾物化**:size=0 的文件与空目录不占流、永无块到达——接收端按清单**显式物化**(创建空文件/空目录);目录 `SetMTime` 在其全部子内容落盘后按**深度逆序**执行(子文件写入会刷新父目录 mtime);完成条件 = 所选块全通过**且**物化完成,续传 journal(§6.12)其后才删除。
...
- **随机 nonce 不改变帧布局**:块密文 = `nonce(12) ‖ ct ‖ tag`,作为一个整体字节串按 `MaxFrameSize` 切成多个 `BLOCK` 帧(`seq` 递增、末帧置 `last`)。接收端按 `index` 重组所有帧 → 还原 `nonce ‖ ct ‖ tag` → 取首 12B 为 nonce → GCM 解密(AAD=suiteByte‖u64_be(index))。
...
- **转发数据面每会话背压**:按 `sessionId` 维护**有界转发队列**(容量 = `InFlightWindow` 块)。接收端请求驱动天然限流,诚实客户端不会溢出;队列满(异常/恶意)即断该会话并回 `ERROR`。中转**始终及时读取发送端 WS**——绝不因单个慢接收端停读,避免队头阻塞殃及其他会话。
...
- 安全:消息大小上限(`MaxFrameSize`)、清单大小上限(`MaxManifestSize`)、每节点清单驻留内存预算(`MaxTotalManifestBytes`,超出拒新 register)、速率限制——**join 点名每 shareId + 每 IP 限速**(join 无鉴权即换取 ≤16 MiB 清单回传,由中转独立回放、不耗发送端上行,放大系数高;自注册大清单再海量 join 同样命中)、**WS Origin 白名单**(允许的前端域由中转部署方配置,自部署中转可放行自部署前端;dev 放行 localhost)。
- **信令优先于转发数据面**:同一 WS 上 `signal`/`bye` 走独立高优先发送队列,不排在 BLOCK 转发帧之后(中转与两端客户端同规)——否则中转回退满载时,队头阻塞会拖慢"热切换到 P2P"这件事本身。
...
- **下载落盘**:优先 File System Access API(流式写目录树);不支持时(Safari/Firefox)单文件回退 StreamSaver / 内存 Blob,**多文件回退为流式 ZIP**(store 不压缩 + Zip64,边解密边写 zip 流,不整包驻留内存;打包只为聚合成单文件下载)。流式路径(StreamSaver / 流式 ZIP)按块号顺序出字节,依赖调度器**有序交付模式**(§6.6)。**落盘/ZIP 条目路径同样按 §6.13 校验**。
...
> **起传策略(产品定义:小文件直接中转,大文件等 P2P 10 秒)**:一切信令/ICE 自用户手势起,而非页面加载(§6.10)。选中总量 ≤ `SmallShareBytes` → 不等 P2P,立即经中转起传,P2P 后台协商、连上即热切换;否则 P2P 赛跑至多 `P2PConnectTimeout`,超时经中转起传、后台 P2P 继续。两种次序对调度器只是通道入池时机不同。
...
- **CLI 接收**:续传状态按**分享**组织(打包流跨多文件,不能按单文件记):输出根下一个 journal **`.wsresume-<指纹前缀>`**(前缀取清单指纹首 8 字节 base64url,**仅做命名空间避撞**——同目录并发下载不同分享互不干扰;文件内仍存**完整清单指纹**,续传校验依据内容而非文件名),记录清单指纹(sealedManifest 的 GCM tag,非秘密)+ 块 bitfield;数据直接按最终相对路径 `WriteRange`,journal 存在即未完成、完成即删除,孤儿 journal 可安全手删。**跨进程重启可续**:指纹一致才续(防跨分享错拼;发送端清单仅 Seal 一次故指纹稳定,§6.3),按 bitfield 只请求缺失块。同一分享并发写同一目录(数据文件本身互撞)属误用,不设文件锁。`.wsresume` 为**保留名前缀**:清单含以其开头的条目 → 拒收(§6.13)。
...
**清单内路径规范(canonical)**:相对路径、`/` 分隔、UTF-8、**NFC 归一化**、无前导 `/`、无盘符/UNC、无 `.`/`..`/空段、无控制字符与 Windows 非法字符(`< > : " | ? * \`)。
...
- **重复检测**:含大小写不敏感折叠与 NFC 折叠后的碰撞 → 报错(否则在 Windows/macOS 落盘会相互覆盖)。

**接收端(落盘前,纵深防御,不信任清单)**:
- **清单结构校验(解密后、下载前)**:`size ≥ 0` 且 size 前缀和 ≤ `MaxStreamBytes`(负 size 与正向回绕破坏同一前缀和推导,属同一攻击面;上限并封 JS float64 精度与恶意 streamLen 撑爆 bitfield/需求集分配);条目 path **唯一**,且大小写折叠 + NFC 折叠后无碰撞(与发送端同规,B7;否则 Windows/macOS 落盘互覆)。
- 再次规范化并确认**解析后不逃逸输出根**(`safeJoin`);拒绝绝对路径、`..`、空段、控制字符;`/` 映射为 OS 分隔符。
- **Windows 保留名与非法字符**(`CON/PRN/AUX/NUL/COM1–9/LPT1–9`、结尾空格或点、`< > : " | ? * \`——`:` 尤须拒绝,NTFS 将其解释为 Alternate Data Stream):**拒绝并明确报错**(M1);可选 sanitization 留后续。
- **路径长度**:组合后的输出绝对路径超出平台上限(Windows `MAX_PATH` 260)→ **拒绝并明确报错**(M1);`\\?\` 长路径前缀留后续。
- **工具保留名(前缀匹配)**:以 `.wsresume` 开头的条目(续传 journal `.wsresume-<指纹前缀>`,§6.12)→ 拒绝并明确报错,防分享内容覆盖/伪造 journal。
- **已存在同名文件**:非续传(无匹配 journal,§6.12)而输出路径已存在同名文件 → **拒绝并明确报错**(不静默覆盖用户数据);强制覆盖开关留后续。
...
| **键石:内存级端到端** | `Sharer.Chunk` 产出喂给 `Receiver.Accept`(mock Source/Sink),重建字节 ⩵ 原始;零真实 IO、零网络,覆盖整条 core 链路 | Go test(注入固定 readSecret + 固定 RNG) |
| **跨实现** | Go ↔ TS 的 crypto / **清单 CBOR** / **数据面二进制** 逐字节一致 | `testvectors/` 黄金向量,双侧各跑一遍 |
...
| 安全 | 校验顺序(篡改密文 → tag 失败)、错位块(AAD 失败)、跨密钥/跨分享 Open 失败、**恶意路径(`../`、绝对、保留名、NFC 碰撞)被拒**、**伪造清单结构(负 size、前缀和超 `MaxStreamBytes`、路径折叠碰撞)被拒**、非续传遇已存在同名 → 拒、文件分享中被改 → 中止 | 故障/篡改/路径注入测试 |
...
| `TagBytes` | 16 | GCM tag |
| `P2PConnectTimeout` | 10 s | P2P 协商赛跑窗口(来自产品定义;仅大分享等待) |
| `SmallShareBytes` | 8 MiB | 选中总量 ≤ 此值 → 跳过 P2P 等待、立即经中转起传(产品定义"小文件直接中转") |
...
| `MaxStreamBytes` | 2⁵³ − 1(≈ 8 PiB) | 打包流总长上限:封 int64 前缀和回绕、JS 安全整数精度、恶意 streamLen 的分配失控;双端校验(§6.13) |
| `MaxTotalManifestBytes` | 512 MiB | 中转单节点清单驻留内存预算(≈ 并发分享数 × 清单大小),超出拒新 register;按节点内存配置(§6.8) |
| `MaxFrameSize` | 64 KiB | 线帧上限(DataChannel 跨浏览器安全值,可协商上调) |
| `InFlightWindow` | 8 块/通道 | 每通道在途请求窗口 |
| `STUNServers` | 公共 STUN(可配置) | NAT 穿透;默认公共 STUN,后续可自建 |
...
| 恶意/异常路径落盘 | 路径穿越、跨平台覆盖 | 清单 canonical(NFC)+ 接收端落盘前校验 + 保留名/碰撞拒绝(§6.13) |
...
| 单个慢接收端拖累中转转发 | 队头阻塞/中转内存膨胀 | 每会话有界队列 + 溢出断会话,不停读发送端 WS(§6.8) |
...
- **序列化 = CBOR(清单)/ 定长二进制(数据面)/ JSON(信令)**;**传输抽象 = FrameChannel + 调度器**;**清单获取 = WS join,无 HTTP 端点**。
- **NFC = M1 必做,用 `golang.org/x/text`,仅 IO 边缘**(跨平台落盘正确性优先)。
...
| **T1.2** | `core/layout`:规范序、offset 前缀和、块↔range 双向映射(纯算术) | T1.1 | 打包流布局 |
...
| **T1.5** | `core/session`:`FrameChannel` 抽象 + 调度器(块协议/选择性子集/续传/**有序交付模式**,§6.6)+ 发送/接收会话(mock 通道) | T1.4 | 调度逻辑(纯单测) |
| **T1.6** | `core/osfs`:遍历(跳符号链接、NFC)、根目录限制 Source/Sink、**size/mtime 漂移检测** | T1.4 | 真实 FS 适配 |
| **T1.7** | **`core/share` 门面(Sharer/Receiver,§6.6)** + `core` 键石:内存级端到端 + **跨语言金标向量**(`-update` 生成并提交) | T1.5,T1.6 | share 门面 + core 闭环 + 向量 |
| **T2.1** | `relay/protocol`(信令消息 + 转发帧包裹的共享类型)+ `relay/signaling`:WS hub(端点 `/v1/ws/<shareId>`,§6.7)、register/join(返回清单)、SDP/ICE 转发、**断线宽限重注册(resumeToken 原像)+ 信令优先队列(§6.8)** | T1.4 | 线协议类型 + 信令服务器 |
| **T2.2** | `relay/forward`:加密字节转发数据面(每会话有界队列背压,§6.8) | T2.1 | 回退通道 |
...
| **T4.2** | CLI `get`:取 nonce 解密校验 + **落盘前路径校验** + 断点续传(`.wsresume-<指纹前缀>` journal)+ `--only` + `--key`(分离密钥,§6.9) | T3.1 | 原生接收端 |
| **T6.1** | E2E:CLI↔CLI 经中转(选择性/续传/篡改/**恶意路径**/分享中改文件即中止/**发送端断线重连续传**/**空文件·空目录物化**/**已存在同名拒绝**) | T4.1,T4.2 | M1a 验收 |

> **验收门 M1a**:`get` 经中转完整正确接收(选择性/续传),哈希一致;文件被改 → 中止;**恶意路径被拒**;覆盖率 ≥70%;**Go 黄金向量自洽(清单/块逐字节稳定)**。
...
> **验收门 M1b**:浏览器经中转完整正确接收,哈希一致;**Go↔TS 黄金向量逐字节一致**;Playwright E2E 绿。
...
> **验收门 M1c(= M1 完成)**:P2P 直连 + 失败回退 + 热切换可用;2 接收端可并发下载同一链接;CI 全绿、覆盖率 ≥70%;黄金向量 Go↔TS 一致。
...
| 块(block) / 帧(frame) | 块=加密/校验/续传单位(AEAD 单位);帧=线传输单位(≤ `MaxFrameSize`) |
...
| `FrameChannel` | 传输层抽象:只搬帧、不懂块;DataChannel 与 relay WS 各实现一个 |
| 确定性编码 | 清单 = canonical CBOR、数据面 = 定长二进制;保证 Go↔TS 逐字节一致 |
| 路径 canonical | 清单路径:相对、`/` 分隔、NFC、无 `..`/盘符;落盘前再校验防穿越 |
...
| B7 | `ValidatePath` 仅拒 `..`/绝对/盘符/反斜杠 | **追加**:NFC 规范化、Windows 保留名(`CON/PRN/AUX/NUL/COM1-9/LPT1-9`、结尾空格/点)、非法字符 `< > : " \| ? *`(`:` = NTFS ADS)、大小写+NFC 折叠碰撞检测、输出路径长度上限(MAX_PATH)、遍历不跟随 reparse point、工具保留名前缀 `.wsresume`(§6.12) | `manifest/manifest.go`、`osfs/osfs.go` |
| B8 | NFC 延后(零依赖) | **引入 `golang.org/x/text`**,在 `manifest`(构建)与 `osfs`(校验)做 NFC;纯加密包当前用不到外部依赖 | `go.mod`、`manifest/`、`osfs/` |
| B9 | `share` 门面方法 `Accept(i, ct)` | 重命名语义为 `Accept(i, blockCT)`(含 nonce);其余签名不变 | `share/share.go` |
...
| B14 | 清单含 `offset`/`streamLen` 字段,接收端"只读不算" | **删除两字段**:双端按 entries 数组顺序对 size 前缀和各自推导(§6.4),数组顺序即流顺序(GCM tag 认证);接收端补 `size ≥ 0` + 前缀和 ≤ `MaxStreamBytes` + path 折叠碰撞校验(§6.13) | `manifest/manifest.go`、`layout/layout.go` |
| B15 | 严格解码直接按当前 schema 解全量,未知版本抛 CBOR 结构错误 | 解码**先宽容探测 `v`**:未知版本 → 明确"请升级"报错;已知版本才按对应 schema 严格解码(§6.4) | `manifest/manifest.go` |

> 金标向量(`testvectors/`)字段相应新增/调整:每块密文项 = `base64(nonce ‖ ct ‖ tag)`;清单项 = `base64(nonce ‖ cbor_gcm)`;并附 TS 侧解码步骤说明(读 nonce → HKDF → GCM Open,AAD=suiteByte‖u64_be(i))。
